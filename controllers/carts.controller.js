...
export default class CartsController {
  saveData = (req, res) =>
    cartsDAO
      .addItemToCart(req.user.data.cartID, req.body.product)
      .then((response) => res.send(response))
  clearCartByID = (req, res) =>
    cartsDAO.clearCart(req.params.cartid).then((response) => res.send(response))
  removeCartItem = (req, res) =>
    cartsDAO
      .removeCartItem(req.params.cartid, req.params.itemid)
      .then((response) => res.send(response))
  ...
  cartCheckout = (req, res) => {
    ...
    cartsDAO
      .cartCheckout(req.user.data.cartID, req.user)
    ...
    cartsDAO.findCartByID(req.user.data.cartID).then((response) => {
      ordersDAO.save({
        ...
        status: "undelivered",
        timestamp: timestamp,
        orderEmail: req.user.data.email,
