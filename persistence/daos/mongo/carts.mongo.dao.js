...
export default class CartMongoDAO extends MongoContainer {
  ...
  async removeCartItem(cartID, itemID) {
    try {
      ...
      const dataFound = findOne.products.find(
      ...
      const data = await this.model.findOneAndUpdate(
        { cartID: cartID },
        {
          $pull: { products: { itemID: itemID } },
          $set: { total: total - dataFound.itemPrice },
        },
        { new: true }
      )
      return { message: "Query successfully resolved", data }
  ...
  async addItemToCart(cartID, productData) {
    try {
      ...
      const data = await this.model.findOneAndUpdate(
        { cartID: cartID },
        { $push: { products: productData }, $set: { total: total } },
        { new: true }
      )
  ...
  async cartCheckout(cartID, userData) {
    try {
      this.findCartByID(cartID).then((response) => {
        const cartRepoInstance = new cartsRepository(
          response.data,
          userData.data
        )
